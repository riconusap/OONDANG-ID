...
<template>
  <section 
    ref="targetContainer"
    :class="[
        'py-20 px-6 bg-white text-wedding-dark transition-all duration-800 ease-out will-change-[opacity,transform]',
  ...
  >
      ...
      <div class="space-y-8 border-l-2 border-wedding-brown/30 ml-3 pl-6 relative">
          <div v-for="(story, index) in stories" :key="story.id" class="relative">
              <div class="absolute -left-[33px] top-1 w-4 h-4 bg-wedding-brown rounded-full border-4 border-white"></div>
