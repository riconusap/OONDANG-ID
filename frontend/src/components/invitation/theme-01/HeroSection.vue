...
const groomName = computed(() => props.invitation?.customTexts?.groomName || authStore.profile?.groom_name || 'LIVIANA');
const brideName = computed(() => props.invitation?.customTexts?.brideName || authStore.profile?.bride_name || 'ADRIAN');
...
const eventDateObj = computed(() => {
    let dateStr = authStore.profile?.event_date || '2026-12-31';
...
<template>
  <section id="hero" class="relative min-h-screen flex flex-col bg-black text-white px-6 pt-16 overflow-hidden">
      ...
      <!-- Bottom Block -->
      <div class="flex-1 w-full mt-10 flex relative -mx-6 w-[calc(100%+3rem)]">
          <!-- Left Vertical Date -->
