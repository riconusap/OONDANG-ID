...
const email = ref('dimas.anissa@example.com')
...
async function handleSubmit() {
  ...
  if (ok) {
    notificationStore.showToast('Selamat datang kembali di oondang.id!', 'Login Berhasil')
    router.push('/')
  } else {
    notificationStore.showError(
      authStore.error || 'Alamat email atau kata sandi tidak cocok. Silakan coba lagi.',
      'Gagal Masuk Akun'
    )
  }
}
...
async function handleGoogleSignIn() {
  ...
  if (result.success) {
    if (result.isFreshAccount) {
      notificationStore.showToast('Selamat datang di oondang.id! Mari lengkapi persiapan awal Anda.', 'Akun Baru Terdaftar')
    ...
    notificationStore.showError(
      authStore.error || 'Gagal masuk dengan akun Google.',
      'Gagal Masuk Akun'
    )
  }
}
...
function handleFillDemo() {
  email.value = 'dimas.anissa@example.com'
  ...
  <div class="min-h-screen bg-canvas flex flex-col justify-center py-12 sm:px-6 lg:px-8 px-4 font-sans">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md text-center">
      ...
      <h1 class="mt-4 text-2xl font-bold text-ink">
        Masuk ke Ruang Rencana Catin
      </h1>
      <p class="mt-1 text-xs text-ink-muted">
        Persiapkan hari bahagia Anda dengan tenang, terstruktur, dan transparan.
      </p>
    </div>
      ...
      <div class="bg-surface py-8 px-6 shadow-sm rounded-2xl border border-border sm:px-8">
        ...
        <div class="relative mb-5">
          ...
          <div class="relative flex justify-center text-xs">
            <span class="bg-surface px-2 text-ink-muted">atau masuk dengan email</span>
          </div>
        </div>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <button
            type="button"
            class="w-full py-2 text-center text-xs text-ink-muted hover:text-primary transition-colors cursor-pointer border border-dashed border-border rounded-lg bg-surface-subtle"
            @click="handleFillDemo"
          >
            Isi Otomatis Akun Demo Catin (Dimas &amp; Anissa)
          </button>
        </form>

        <div class="mt-6 text-center text-xs text-ink-muted border-t border-border pt-4">
          Belum memiliki akun calon pengantin?
          <router-link to="/auth/register" class="text-primary font-semibold hover:underline ml-1">
            Daftar Sekarang
          </router-link>
        </div>
      </div>
