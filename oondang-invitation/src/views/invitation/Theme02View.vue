...
import CoverOverlay from '@/components/invitation/theme-02/CoverOverlay.vue';
import HeroSection from '@/components/invitation/theme-02/HeroSection.vue';
import QuoteSection from '@/components/invitation/theme-02/QuoteSection.vue';
import ProfilesSection from '@/components/invitation/theme-02/ProfilesSection.vue';
import EventSection from '@/components/invitation/theme-02/EventSection.vue';
import GallerySection from '@/components/invitation/theme-02/GallerySection.vue';
import LoveStorySection from '@/components/invitation/theme-02/LoveStorySection.vue';
import GiftSection from '@/components/invitation/theme-02/GiftSection.vue';
import WishesSection from '@/components/invitation/theme-02/WishesSection.vue';
...
<template>
  <div :class="['bg-sage-50 text-zinc-800 font-wedding-sans flex justify-center lg:justify-end m-0 relative', isPreview ? 'h-full' : 'min-h-screen']">
      
      <!-- Desktop Background Panel (Hidden on mobile) -->
      <div class="hidden lg:flex fixed inset-y-0 left-0 right-[480px] bg-cover bg-center bg-no-repeat flex-col justify-center items-center text-white"
           :style="{ backgroundImage: `url('${invitation?.customImages?.cover_bg || 'https://placehold.co/1200x800/111111/4a4036?text=Background'}')` }">
          <!-- Dark overlay -->
      ...
      <div class="w-full lg:w-[480px] flex justify-center z-10 shrink-0">
          ...
          <div v-if="invitation?.musicUrl" class="fixed bottom-6 right-6 z-50">
              <button 
                  @click="toggleAudio" 
                  class="w-12 h-12 bg-sage-600 text-white rounded-full flex items-center justify-center shadow-2xl transition-transform hover:scale-110 border-2 border-sage-200"
